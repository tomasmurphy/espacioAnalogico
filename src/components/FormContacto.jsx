...
export const FormContacto = ({taller, handleClose}) => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_0n5st72', 'template_b0v2x4p', form.current, '4Jc5CAc5mEdng1aue')
      .then((result) => {
        Swal.fire({
  ...
  <input type="hidden" name="nov_message" value={nov ? "Quiero recibir novedades Espacio Analógico" : "No se suscribió"} />
  <span className="checkmark ms-2"></span>
  <span className="ms-4 labelCheck">Quiero recibir novedades de Espacio Analógico</span>
</label>
          </div>
